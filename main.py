...
def new_user():
    ...
    if request.method == 'POST':
        ...
        cur = mySQL.connection.cursor()
        cur.execute("""
            INSERT INTO usuario (nombre, apellido, edad, ci, telefono, email, usuario, password, sexo, imagen, fecha, admin, fecha_codigo)
            VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
        """, (nombre, apellido, edad, cedula, celular, email, user, password, sexo, imagen_filename, fecha_actual, rol, fecha_codigo_def))
        mySQL.connection.commit()
...
def listar_casos():
    ...
    cur.execute("""
            SELECT * FROM caso 
            WHERE activo = 1 AND f_procesada = 'no_procesado' AND usuario_id = %s
        """, (id_usuario_sesion,))
